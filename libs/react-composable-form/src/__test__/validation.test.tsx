    ...
    it("can disable validation", async () => {
      const Form = createForm((options) =>
        options
          .validateOn()
          .schema(z.object({ foo: z.string().min(3) }))
          .type(z.string(), ({ onChange, value = "", errors = [], name }) => (
            <>
              <input
    ...
    it("can display field errors on focus", async () => {
      const Form = createForm((options) =>
        options
          ...
          .type(
            z.string(),
            ({ onFocus, onChange, value = "", errors = [], name }) => (
              <>
                <input
                  onFocus={onFocus}
                  onChange={(e) => onChange?.(e.target.value)}
    ...
    it("can display field errors on blur", async () => {
      const Form = createForm((options) =>
        options
          ...
          .type(
            z.string(),
            ({ onBlur, onChange, value = "", errors = [], name }) => (
              <>
                <input
                  onBlur={onBlur}
                  onChange={(e) => onChange?.(e.target.value)}
    ...
    it("can display field errors on blur or change", async () => {
      const Form = createForm((options) =>
        options
          ...
          .type(
            z.string(),
            ({ onChange, onBlur, value = "", errors = [], name }) => (
              <>
                <input
                  onBlur={onBlur}
                  onChange={(e) => onChange?.(e.target.value)}
    ...
    it("can reset", async () => {
      const Form = createForm((options) =>
        options
          .validateOn("change")
          .schema(z.object({ foo: z.string().min(3) }))
          .type(z.string(), ({ onChange, value = "", errors }) => (
            <>
              <input
                onChange={(e) => onChange?.(e.target.value)}
                value={value}
