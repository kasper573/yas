...
Vanilla Extract is encapsulated in the `@yas/css` package, so you don't need to install it yourself.

```tsx
import { style, recipe } from "@yas/css";

const myStyle = style(...); // Equivalent of using {style} from vanilla-extract/css
const myRecipe = recipe(...); // Equivalent of using {recipe} from vanilla-extract/recipes
```

Our [Atomic CSS framework](https://vanilla-extract.style/documentation/packages/sprinkles/) is available as `atoms`:

```tsx
import { atoms } from "@yas/css";
const myStyle = atoms({
  color: "red",
  fontSize: 16,
});
...
A convenience react component factory is available as `styled`:
